...
def extract_text_from_file(file_path, use_ocr=False):
    ...
    try:
        elements = partition(filename=str(file_path),ocr_languages="eng")
        text = ""
        for element in elements:
             if isinstance(element, Text) or isinstance(element,Table):
                 text+=element.text + "\n"
        if not text.strip() and use_ocr and file_path.suffix.lower() == ".pdf":
              ...
              logging.warning(f"Failed to extract text from {file_path} using unstructured.")
              return None
