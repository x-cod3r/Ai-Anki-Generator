...
from workflow_utils import process_files, chat_with_pdf, create_anki_deck, extract_text_from_pdf_no_ocr
import time
import functools
...
class AppGUI:
    ...
    def run_processing(self, input_path, pages_per_pdf, use_ocr, pdf_engine, data_file, anki_output, ai_model, api_key):
        ...
        anki_output_path = os.path.join(script_dir, anki_output)
        logging.info(f"Starting run_processing with input_path: {input_path}, data_file: {data_file_path}, anki_output: {anki_output_path}, model: {ai_model}, api_key:{api_key}")

        try:
            # 1. Process files into PDFs
            logging.info("Starting PDF processing...")
            process_files(input_path, script_dir, pages_per_pdf, use_ocr, pdf_engine)
            logging.info("PDF processing complete.")

            # 2. Generate Flashcards from PDFs and save to text file
            # Assuming you want to process all generated PDFs
            pdf_files = [file for file in Path(script_dir).glob("*.pdf")]
            if pdf_files:
               self.pending_chats = len(pdf_files) #Update the counter of open chats.
               for pdf_file in pdf_files:
                    logging.info(f"Found PDF for chat: {pdf_file}")
                    self.master.after(0, self.open_chat_window, str(pdf_file), data_file_path, anki_output_path, ai_model, api_key)  # Use master after
            else:
